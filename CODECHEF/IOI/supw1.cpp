	...
	//	if(!vp[i].second && !vp[i+1].second && !vp[i+2].second){
			...
			if(i>0){
				if(t-last_pos<=3){  //changed to max
					if(t-last_pos!=0){
						int tmp=(t==i)?(vp[i+1].first>vp[i+2].first)?(!(vp[i+2].second))?(i+2):(i+1):(!(vp[i+1].second))?(i+1):(i+2):(t==i+1)?(vp[i].first>vp[i+2].first)?
						(!(vp[i+2].second))?(i+2):i:!(vp[i].second)?i:(i+2):(vp[i].first>vp[i+1].first)?(!(vp[i+1].second))?(i+1):i:!(vp[i].second)?i:(i+1);
						if((vp[last_pos].first+vp[t].first) >=  vp[tmp].first){
							if((vp[last_pos].first+vp[t].first) ==  vp[tmp].first && t>tmp){
								v.push_back(t);
								last_pos=t;
