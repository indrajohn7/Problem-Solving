...
bool IsSwapping(int** grid,int r,int c,int n);
...
bool SolveSudoku(int** grid)
{
    ...
    {
        // if looks promising
        if (isSafe(grid, row, col, num) && IsSwapping(grid, row, col, num)) // Call IsSwapping for each entry
        {
...
void DoSwap(int** arr,int** temp,int row,int col,int direction,int num)
{
	for(int i=0;i<N;i++)
		for(int j=0;j<N;j++)
			temp[i][j]=arr[i][j];
	...
	temp[row][col]=num;
	int r,c;
	r=row-(row%(int)(sqrt(N)));
	c=col-(col%(int)(sqrt(N)));
	if(direction==0)
		return;
		...
		for(int i=0;i<(int)(sqrt(N));i++){
			for(int j=0;j<(int)(sqrt(N));j++)
				swap(temp[i+r][j+c],temp[i+r-(int)(sqrt(N))][j+c]);
		}
	}
		...
		for(int i=0;i<(int)(sqrt(N));i++){
			for(int j=0;j<(int)(sqrt(N));j++)
				swap(temp[i+r][j+c],temp[i+r][j+c-(int)(sqrt(N))]);
		}
	}
...
bool IsSwapping(int** arr,int row,int col,int num)
{
	//int** temp=arr;
	int** temp=new int*[N];
	for(int p=0;p<N;p++)
		temp[p]=new int[N];
			...
			left=3;
			right=4;
			if(row >= 0 && row < (int)sqrt(N)){
				up=0;
			}if(col>=0 && col < (int)sqrt(N))
				left=0;
			if(row >= (N-(int)sqrt(N)))
				down=0;
			if(col >= (N-(int)sqrt(N)))
			...
			if(up!=0){
				DoSwap(arr,temp,row,col,up,num);
								
				if(UsedInRow1(temp, row-(int)(sqrt(N)),col, num) ||
					UsedInCol1(temp, row-(int)(sqrt(N)),col, num))
						return false;
			
			}
			
			if(down!=0){
				DoSwap(arr,temp,row,col,down,num);
			...
			if(left!=0){
				DoSwap(arr,temp,row,col,left,num);
			
				if(UsedInRow1(temp, row,col-(int)(sqrt(N)), num) ||
					UsedInCol1(temp, row,col-(int)(sqrt(N)), num))
						return false;
			}
			
			if(right!=0){
				DoSwap(arr,temp,row,col,right,num);
