...
int verify(char* arr,int a,int b)
{
	int res=0;
	for(int i=a;i<=b;i++){
		if(arr[i]=='0')
...
int main()
{
	...
	for(int i=0;i<nd.size();i++){
		...
		if(verify(time,nd[i].start_time-1,nd[i].end_time-1)==(nd[i].end_time-nd[i].start_time+1)){
			...
			n.price=nd[i].price;
			out.push_back(n);
			memset(time+nd[i].start_time-1,'1',(nd[i].end_time-nd[i].start_time+1));
