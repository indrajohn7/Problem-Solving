...
class graph
{
    public:
        ...
        void addedge(const int from, const int to, double cost);
          ...
          //      void BFS(vertex*,int);
                void Print_Path(vertex*,vertex*,int*,int*,int,int**);
...
void graph::addedge(const int from, const int to, double cost)
{
 ...
 void graph::Print_Path(vertex* s,vertex* e,int* flag,int* i,int K,int** route)
 {
        if(e==s)
               return; // cout<<Source->name<<"\n";
         else if(e->parent==NULL){
			 *flag=1;
			 return;
		 }else{
			 
			 if(*i<K-1 && e->name==route[*i+1][0] && vec.size()<=1){
						++(*i);
						vec.push_back(*i);
					}else if(e->name==route[*i][1] && *i==vec.front() && vec.size()==2)
							vec.pop_back();
                 Print_Path(s,e->parent,flag,i,K,route);
                // cout<<ver->name;
         }
	...
	for(iter=0;iter<Num_Test;iter++){
		...
		for(int i=0;i<K;i++){
			...
			while(vec.size()){
				...
				bool end=false;
				g.Print_Path(s,e,&flag,&i,K,route);
