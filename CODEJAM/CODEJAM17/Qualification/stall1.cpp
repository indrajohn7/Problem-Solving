...
long long int interval_check(vector<long long int> vec,long long int* ind)
{
	...
	/*	
		...
		//		n++;
			...
			for(int i=1;i<=n;i++){
				...
				while((vec.size()-2)<K){
					long long int ind=0;
					long long int idx=interval_check(vec,&ind);
					int pos=idx+((vec[ind+1]-idx)/2);
					vec.push_back(pos);
